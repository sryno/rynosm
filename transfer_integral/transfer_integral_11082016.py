...
def TI(argv):
    ...
    if (len(argv) != 2) and (len(argv) != 3) and (len(argv) != 7):
        print >> sys.stderr, "Usage:", argv[0], "system_prefix [g09/orca/qchem] optional([homo1] [lumo1] [homo2] [lumo2])"
        print >> sys.stderr, "\"", argv[0], "-h\" will print a help file."
    ...
    if len(argv) == 3:
        ...
        print >> sys.stderr, "There is an error reading the input homo and lumos"
        print >> sys.stderr, "Exiting..."
        sys.exit(1)
    ...
    if argv[2] == 'g09':
        ...
        print >> sys.stderr, "Unrecognized program type."
        print >> sys.stderr, "Exiting..."
        sys.exit(1)
...
def TI_header():
    ...
    print >> sys.stdout, "################################################################################"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "##                        Electronic Coupling Program                         ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "##                               Sean M. Ryno                                 ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "##                             November 8, 2016                               ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Usage: transfer_integral.py system_name [g09/orca/qchem]                   ##"
    print >> sys.stdout, "##                             optional([homo1] [lumo1] [homo2] [lumo2])      ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Here, homo[1/2], lumo[1/2] are the upper and lower bounds of orbital of    ##"
    print >> sys.stdout, "##       interest for monomer [1/2] using the orbital numbering of            ##"
    print >> sys.stdout, "##       monomer [1/2]. All possible combinations of couplings will be        ##"
    print >> sys.stdout, "##       calculated.                                                          ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Note: This program makes use of components of the gauss_parse.py           ##"
    print >> sys.stdout, "##       program of J. J. Goings                                              ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Note: The output files from calculations of dimer and monomers must        ##"
    print >> sys.stdout, "##       include molecular orbital coefficients and the dimer overlap matrix  ##"
    print >> sys.stdout, "##       in addition to standard output                                       ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Note: Program auto-detects restricted or unrestricted calculations         ##"
    print >> sys.stdout, "##       and provide respective couplings                                     ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Note: For combined Gaussian jobs, the dimer calculation must come first.   ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Note: Gaussian must adopt the following naming convention:                 ##"
    print >> sys.stdout, "##       dimer: *_A_B.fchk & *_A_B.log                                        ##"
    print >> sys.stdout, "##       monomer 1: *_A.fchk                                                  ##"
    print >> sys.stdout, "##       monomer 2: *_B.fchk                                                  ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Gaussian keywords: # b3lyp/6-31g** nosymm pop=full iop(3/33=1,3/32=2)      ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Additional Orca keywords: print[ P_mos ] 1 & print[ P_overlap ] 1          ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Q-Chem $rem section must include:                                          ##"
    print >> sys.stdout, "##       iprint            200                                                ##"
    print >> sys.stdout, "##       print_orbitals    1000000                                            ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "## Based upon: E. F. Valeev, V. Coropceanu, D. A. da Silva Filho, S. Salman,  ##"
    print >> sys.stdout, "##             J. L. Bredas, J. Am. Chem. Soc. 128, 9882-9886 (2006)          ##"
    print >> sys.stdout, "##                                                                            ##"
    print >> sys.stdout, "################################################################################"
    print >> sys.stdout, "################################################################################"
...
def TI_g09_restricted(system_name, dimer_log, d_fchk_line, mon1_fchk_line, mon2_fchk_line, homo_mon1, lumo_mon1,
                        homo_mon2, lumo_mon2):
    # Get dimer values
...
def TI_g09_unrestricted(system_name, dimer_log, d_fchk_line, mon1_fchk_line, mon2_fchk_line, homo_mon1, lumo_mon1,
                        homo_mon2, lumo_mon2):
    # Get dimer values
    nbf_d = g09_get_basis_functions(d_fchk_line)
...
@jit
def g09_calculate_unrestricted_coupling(system_name, f_a, f_b, s, homo_a_mon1, homo_b_mon1, lumo_a_mon1, lumo_b_mon1,
                                        homo_a_mon2, homo_b_mon2, lumo_a_mon2, lumo_b_mon2, cfrag_a, cfrag_b,
                                        nbf_mon1):
    # Calculate intermolecular couplings
...
def qchem_get_MO_coefficients(qchem_output, nbf):
    ...
    while count < nbf * nbf:
        if ("eigenvalues" in qchem_output[line_index + icount]):
            ...
            break
        elif (qchem_output[line_index + icount].split()[1] == "C") or \
                (qchem_output[line_index + icount].split()[1] == "H") or \
                (qchem_output[line_index + icount].split()[1] == "S"):
            mo_line = qchem_output[line_index + icount]
            ccounter = bcounter
            #for i in range(6):                                     //Used in a previous version that assumed that there was white space between values
            #    c[acounter, ccounter] = float(mo_line[i + 4])
            #    ccounter += 1
            #    count += 1
            dcounter = 0
